...
const APP_ORIGIN = "https://compta.axe-dossier.fr";
  ...
  async (req, res) => {
    ...
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${APP_ORIGIN}/merci.html`,
        cancel_url: `${APP_ORIGIN}/cloture-resultat.html?id=${encodeURIComponent(closureId || "")}`,
...
function detectAccountingEntries(balanceRows, grandLivreRows, amortissementRows = [], empruntRows = [], closure = {}) {
  ...
  const allRows = [...balanceRows, ...grandLivreRows];
  const hasAccount = prefixes => allRows.some(row => accountStarts(row, prefixes));
  const getBalanceAmount = prefixes => {
    const row = findBalanceRow(balanceRows, prefixes);
    return row ? getAmount(row) : 0;
  };

  if (hasAccount(["21", "28"])) controls.push({ type: "immobilisation_detected", label: "Immobilisation ou amortissement détecté", level: "info" });
  if (hasAccount(["164", "661"])) controls.push({ type: "loan_detected", label: "Emprunt ou intérêts détectés", level: "info" });
  if (hasAccount(["706", "707"])) controls.push({ type: "revenue_detected", label: "Chiffre d'affaires détecté", level: "info" });
  ...
  if (hasAccount(["486"]) && answers.cca === "yes") {
    const ccaRows = grandLivreRows.filter(row => {
      const compte = getCompte(row);
      const text = getRowText(row);
      return compte.startsWith("486") && (text.includes("cca") || text.includes("charge constatee") || text.includes("charges constatees") || text.includes("periode suivante"));
    });

    entries.push(...(ccaRows.length ? makeLedgerEntries(ccaRows, {
      label: "CCA",
      debit: "486000",
      credit: "616000",
  ...
  if (hasAccount(["487"]) && answers.cca === "yes") {
    ...
    if (pcaRows.length) {
      pcaRows.filter(row => getCompte(row).startsWith("487")).forEach(row => {
  ...
  if (answers.fournisseurs === "yes") {
    ...
    capRows.forEach(row => {
      ...
      if (compte.startsWith("428") || compte.startsWith("438") || text.includes("conges payes") || text.includes("congés payés")) return;
  ...
  if (answers.stocks === "yes") {
    ...
    stockConfigs.forEach(config => {
      const rows = grandLivreRows.filter(row => config.prefixes.some(prefix => getCompte(row).startsWith(prefix)));
      rows.forEach(row => {
        stockFound = true;
        entries.push({
          journal: "OD",
  ...
  if (hasAccount(["131", "139", "777"])) {
    const subventionRow = findFirstRowByPrefixes(balanceRows, ["131"]) || findFirstRowByPrefixes(grandLivreRows, ["131"]);
    const repriseRow = findFirstRowByPrefixes(balanceRows, ["139"]) || findFirstRowByPrefixes(grandLivreRows, ["139"]);
    const quotePartRow = findFirstRowByPrefixes(balanceRows, ["777"]) || findFirstRowByPrefixes(grandLivreRows, ["777"]);
    ...
    entries.push({
      ...
      credit: "—",
      amount: retainedAmount,
      justification: `Subvention d'investissement détectée.\n\nCompte 131 - Subvention d'investissement : ${subventionAmount || "?"} €\nCompte 139 - Subventions inscrites au résultat : ${repriseAmount || "?"} €\nCompte 777 - Quote-part virée au résultat : ${quotePartAmount || "?"} €\n\n${quotePartAmount || repriseAmount ? "Une reprise de subvention semble comptabilisée. Vérifier le plan de reprise et la cohérence avec l'amortissement de l'immobilisation financée." : "Aucune quote-part de subvention n'a été détectée. Vérifier si une reprise devait être comptabilisée sur l'exercice."}`,
  ...
  if (answers.immo === "yes") {
    const leasingRows = grandLivreRows.filter(row => {
      const compte = getCompte(row);
      const text = getRowText(row);
      return compte.startsWith("612") || text.includes("credit bail") || text.includes("crédit bail") || text.includes("leasing") || text.includes("loyer vehicule") || text.includes("loyer véhicule") || text.includes("photocopieur") || text.includes("location materiel") || text.includes("location matériel");
    });
    ...
    if (leasingRows.length) controls.push({ type: "leasing_detected", label: "Crédit-bail ou leasing détecté", level: "info" });
  }

  if (hasAccount(["455"])) {
    const associateRows = allRows.filter(row => getCompte(row).startsWith("455"));
  ...
  if (hasAccount(["471", "472"])) {
    const rawWaitingRows = allRows.filter(row => getCompte(row).startsWith("471") || getCompte(row).startsWith("472"));
  ...
  if (hasAccount(["23"])) {
    const constructionRows = allRows.filter(row => getCompte(row).startsWith("23"));
    ...
    const totalConstruction = uniqueConstructionRows.reduce((total, row) => total + (getAmount(row) || 0), 0);
  ...
  if (hasAccount(["428"]) && answers.paie === "yes") {
    ...
    entries.push({
      ...
      credit: "438600",
      amount: socialAmount,
      justification: payrollRate ? `Charges sociales estimées à partir du taux historique détecté : ${Math.round(payrollRate * 100)} %.` : "Charges sociales sur congés payés à calculer : comptes 641/645 insuffisants.",
  ...
  if (entries.length === 0) {
    anomalies.push({ type: "no_entries_generated", label: "Aucune écriture générée selon les réponses fournies", level: "info" });
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (amortissementRows.length) {
        controls.push({ type: "amortissements_loaded", label: "Tableau d’amortissement chargé", count: amortissementRows.length });
      }

      if (empruntRows.length) {
        controls.push({ type: "emprunt_loaded", label: "Tableau d’emprunt chargé", count: empruntRows.length });
      }
